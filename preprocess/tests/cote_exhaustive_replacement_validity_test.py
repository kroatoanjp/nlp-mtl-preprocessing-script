...
# that of the original MTL script for the vol. 7 and 8 of the COTE LN
...
DOCS_FOLDER = "preprocess/tests/docs/cote"
REPLACEMENT_TABLE_FILENAME = "replacement_table/cote.json"
...
def main():
    fugashi_tokenizer = FugashiTokenizer()
    ...
    old_preprocess_factory = build_original_preprocess_factory(replacement_table)
    new_preprocess_factory = build_nlp_preprocess_factory(replacement_table, spacy_tokenizer)
    run_validity_test(
        fileset=fileset,
        old_preprocess_name="Basic MTL Preprocess",
